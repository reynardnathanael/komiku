...
$sql = "SELECT c.id, c.isi, DATE_FORMAT(c.timestamp, '%d %b %Y') as timestamp, u.user_name, u.avatar FROM comments c INNER JOIN users u ON c.user_id = u.user_id WHERE c.comic_id=? AND c.parent_id is NULL ORDER BY c.timestamp ASC";
$stmt = $c->prepare($sql);
